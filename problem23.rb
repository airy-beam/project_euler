...
abundants = []
numbers_can_be_sum = []
1.upto(UPPER_LIMIT) { |n| if n.is_abundant? then abundants << n end }
#print "Lists done", "\n"
0.upto(abundants.size - 1) { |i|
  #if ( i % 1000 == 0 ) then print i, "\n" end
  0.upto(abundants.size - 1) { |j|
    numbers_can_be_sum[ abundants[i] + abundants[j] ] = true
  }
}

sum = 0
1.upto(UPPER_LIMIT) { |n| 
  if ( not numbers_can_be_sum[ n ] ) then sum += n end
}
print sum, "\n"

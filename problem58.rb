...
#Idea is not to build the whole spiral, but to calculate values
#on two diagonals
...
begin
  n += 2
  if ( n * n ).is_prime? then primes_count += 1 end
  if ( n * n - n + 1 ).is_prime? then primes_count += 1 end
  if ( n * n - 2 * n + 2 ).is_prime? then primes_count += 1 end
  if ( n * n - 3 * n + 3 ).is_prime? then primes_count += 1 end
  numbers_on_diagonals += 4
end until ( primes_count.to_f / numbers_on_diagonals < 0.1 )

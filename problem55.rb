...
#is Lychrel until proven otherwise. 
...
#has managed so far to map it to a palindrome. In fact, 10677 is 
#the first number to be shown to require over fifty iterations 
...
class Integer
  ...
  def is_palindrome?
    if self == self.to_s.reverse.to_i then true else false end
  end
  ...
  def is_Lychrel?
    ...
    loop do
      ...
      iterations += 1
      if number.is_palindrome? then return false end
      if iterations >= 50 then return true end
    end
  end
...
count = 0
1.upto(9_999) { |n| count += 1 if n.is_Lychrel? }
print count
